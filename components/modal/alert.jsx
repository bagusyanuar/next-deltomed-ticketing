...
function ModalAlert({ isOpen, onClose, message }) {
    ...
    return (
        <div className={`${isOpen ? '' : 'hidden'} absolute z-50 inset-0 bg-black bg-opacity-30 h-screen w-full flex justify-center items-start md:items-center pt-10 md:pt-0`}>
            <div className={`${isShow ? '' : 'opacity-0 scale-50'} transform  relative w-10/12 md:w-1/4 bg-white rounded-lg shadow-lg transition-opacity transition-transform ease-in-out duration-[300ms]`}>
